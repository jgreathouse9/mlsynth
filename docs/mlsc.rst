...
mlSC works on a two-level panel, so its symbols extend the canon with one
extra index for the disaggregate (sub-unit) level. Let :math:`j = 1` denote
the treated aggregate unit (the single state where the policy lands), with
all aggregates :math:`\mathcal{N} \coloneqq \{1, \dots, N\}` and the donor
pool of control aggregates
:math:`\mathcal{N}_0 \coloneqq \mathcal{N} \setminus \{1\}`. We index a
generic control aggregate by :math:`s \in \mathcal{N}_0`; each aggregate
:math:`s` contains :math:`C_s` disaggregate sub-units (e.g. counties),
indexed :math:`c = 1, \dots, C_s`. Time runs over
...
:math:`\mathcal{T}_2 \coloneqq \{t \in \mathcal{T} : t > T_0\}`, and is
absorbing thereafter (treatment never turns off).

Outcomes carry the sub-unit index: :math:`y_{sct}` is the disaggregate
outcome of sub-unit :math:`c` in aggregate :math:`s` at time :math:`t`, and
:math:`y_{st}` the aggregate outcome. The treated aggregate's series is
:math:`\mathbf{y}_1` with entries :math:`y_{1t}`. The disaggregate donor
pool stacks every sub-unit of every control aggregate into the donor matrix
:math:`\mathbf{Y}_0 \in \mathbb{R}^{T \times N_0}` with
:math:`N_0 \coloneqq \sum_{s \in \mathcal{N}_0} C_s` columns (one per
disaggregate donor). Population aggregation weights :math:`v_{sc}` sum to one
within each aggregate. Disaggregate donor weights are
:math:`\mathbf{w} \in \mathbb{R}^{N_0}` with entries :math:`w_{sc}`,
constrained to the unit simplex
:math:`\Delta^{N_0} \coloneqq \{\mathbf{w} \in \mathbb{R}_{\ge 0}^{N_0} :
\|\mathbf{w}\|_1 = 1\}`; the optimiser is :math:`\mathbf{w}^\ast`, and
:math:`w_s \coloneqq \sum_{c} w_{sc}` is the implied aggregate weight on
control aggregate :math:`s`. The penalty strength is :math:`\lambda \ge 0`.
...
:math:`\sum_{c} v_{sc} = 1` for every aggregate. The weights
:math:`v_{sc}` are observable to the analyst (uniform by default;
population shares are the canonical alternative).

   *Remark.* This is what makes the estimand at the aggregate level a clean
   function of the disaggregate potential outcomes -- the disaggregate
   weights :math:`\mathbf{w}` can target the aggregate path
   :math:`\mathbf{y}_1` exactly because the two levels are tied by a known
   linear map. Outcomes that aggregate non-linearly (a rate, a log, an order
   statistic) break the link and the disaggregate data no longer reconstructs
   the aggregate target.

A2 (binary, sharp, absorbing treatment at the aggregate
level). A single aggregate unit (:math:`j = 1`) is treated; the
treatment indicator :math:`d_{sct}` is 1 iff :math:`s = 1` and
...
the staggered-aware alternatives).
   ...
   *Remark.* The single-cohort, never-reversed design is what lets the whole
   pre-period :math:`\mathcal{T}_1` serve as untreated training data for one
   target series :math:`\mathbf{y}_1`. Staggered or reversible treatment
   would contaminate the donor pool with treated outcomes; the config
   validator therefore enforces a single cohort by construction.
...
breaks identification at the level mlSC operates on.
   ...
   *Remark.* Because the weights are fit on sub-unit series rather than the
   ...
   :math:`\mathbf{Y}_0`, biasing the counterfactual at the very level mlSC
   exploits.
...
deviations :math:`\eta_{sc}`.

   *Remark.* This is what licenses the variance-decomposition recipe
   (Appendix G) feeding the heuristic
   :math:`\widehat{\lambda} = 2 \widehat{\sigma}_\varepsilon^2 /
   \widehat{\sigma}_y^2`. The model also implies the four-way MSE
   ...
   noise). When the disaggregate variation departs from this clean
   state-effect-plus-iid-shock structure, the heuristic mis-prices the
   flexibility.

A5 (feasibility / convex hull for dGSC-AD). The treated
aggregate's pre-period trajectory lies in the convex hull of the
disaggregate donor pool :math:`\{y_{sct}\}_{s \in \mathcal{N}_0, c}`. With
disaggregation, the donor pool is :math:`N_0 = \sum_{s \in \mathcal{N}_0} C_s`
units wide -- typically an order of magnitude larger than the aggregate-level
hull (only :math:`|\mathcal{N}_0|` control aggregates wide) -- so this is easier to satisfy
than canonical SC's hull condition, which is the main empirical
motivation for the estimator.

   *Remark.* The penalty term is what stops the expanded hull from
   degenerating into a non-unique solution at zero pre-period RMSE: a wide
   disaggregate pool can fit the pre-period in many ways, and the shrinkage
   toward population-share-times-aggregate-weight selects among them. When
   even the :math:`\lambda = 0` fit leaves a large pre-period RMSE, the
   treated aggregate is structurally outside the hull and mlSC cannot help
...
with the data-driven :math:`\widehat{\lambda}` picking the sweet spot.
...
(a) Linear aggregation with known weights (A1). The
    ...
    *Plausibly violated when* the outcome is a ratio (rate,
    ...
    :math:`\sum_{c} v_{sc} y_{sct}` directly from ``df_disagg``
...
(c) SUTVA at the disaggregate level (A3). Disaggregate
    ...
    *Plausibly violated when* counties within a donor state
    ...
    whether the implied :math:`w_{sc}` and pre-RMSE
...
(d) Hierarchical latent-factor outcome model (A4). The
    ...
    :math:`\widehat{\lambda}` is derived under a specific
...
The treated aggregate is :math:`j = 1`; control aggregates are indexed by
:math:`s \in \mathcal{N}_0`, each containing :math:`C_s` disaggregate
sub-units indexed :math:`c = 1, \dots, C_s`. Time periods are
...
(``dGSC``) class, parametrized by a weight array
...
.. math::
   ...
   \tau_t^{\,\text{dGSC}}
   \;=\;
   \sum_{c_0 = 1}^{C_1} v_{1 c_0} \left(
       y_{1 c_0 t}
       -
       \sum_{s \in \mathcal{N}_0} \sum_{c = 1}^{C_s} W_{c_0 s c} \, y_{sct}
   \right),
...
Equation 5.2), the weight array collapses to a single vector
:math:`\mathbf{w}` over all disaggregate control units, with entries
:math:`w_{sc}`. Let
:math:`w_s = \sum_{c} w_{sc}` denote the implied aggregate weight
on donor aggregate :math:`s`. The mlSC estimator solves
   ...
   \underbrace{
     \sum_{t = 1}^{T_0}
       \left(
         y_{1t}
         -
         \sum_{s \in \mathcal{N}_0} \sum_{c = 1}^{C_s}
           w_{sc} \, y_{sct}
       \right)^{2}
   }_{\text{pre-treatment fit}}
   \;+\;
   \lambda \, \widehat{\sigma}_y^2
   \underbrace{
     \sum_{s \in \mathcal{N}_0} \sum_{c = 1}^{C_s}
       \left(
         w_{sc} - v_{sc} \, w_s
...
:math:`w_{sc} = v_{sc} \, w_s`, which collapses the estimator to
...
:math:`\widehat{\sigma}_y^2` keeps :math:`\lambda` itself scale-invariant
across panels.
...
:math:`\mathbf{w}_s \in \mathbb{R}^{C_s}` be the within-block weight
vector and :math:`\mathbf{v}_s \in \mathbb{R}^{C_s}` the corresponding
population weights. Defining
:math:`\mathbf{A}_s = \mathbf{I} - \mathbf{v}_s \mathbf{1}^\top`,
the within-block contribution is

.. math::

   \sum_{c = 1}^{C_s} \left( w_{sc} - v_{sc} \, w_s \right)^{2}
   \;=\;
   \| \mathbf{A}_s \, \mathbf{w}_s \|_2^2
   \;=\;
   \mathbf{w}_s^\top \mathbf{Q}_s \, \mathbf{w}_s,
...
.. math::
   ...
   \mathbf{Q}_s
   \;=\;
   \mathbf{A}_s^\top \mathbf{A}_s
   \;=\;
   \mathbf{I} \;-\; \mathbf{v}_s \mathbf{1}^\top \;-\; \mathbf{1} \mathbf{v}_s^\top
   \;+\; \| \mathbf{v}_s \|_2^{\,2} \, \mathbf{1} \mathbf{1}^\top.

The full penalty matrix
:math:`\mathbf{Q} \in \mathbb{R}^{N_0 \times N_0}`, with
:math:`N_0 = \sum_s C_s` the total number of disaggregate control units,
is block-diagonal across :math:`s` with blocks :math:`\mathbf{Q}_s`.
:math:`\mathbf{Q}_s` is symmetric positive semidefinite with
:math:`\mathbf{v}_s` in its kernel (any weight vector proportional to
:math:`\mathbf{v}_s` incurs zero penalty), so the mlSC optimization remains
a convex QP and admits standard ``cvxpy`` solvers. :mod:`mlsynth` uses
``SCS`` by default; this matches the reference implementation and avoids any
...
* ``'heuristic'`` — the closed-form rule from Section 5.2 of the
  ...
  .. math::
     ...
     \widehat{\lambda}
     \;=\;
     2 \, \widehat{\sigma}_\varepsilon^2 \,/\, \widehat{\sigma}_y^2,

  derived from the oracle optimal :math:`\lambda^\ast` in a stylized
...
non-treated averages :math:`\widehat{\sigma}_\varepsilon^2 =
\mathrm{mean}_s \widehat{\mathrm{Var}}(\varepsilon)_s` and
:math:`\widehat{\sigma}_y^2 = \mathrm{mean}_s \widehat{\mathrm{Var}}(y)_s`.
...
Given the optimal :math:`\mathbf{w}^\ast`, the aggregate counterfactual is
...
\sum_{t \in \mathcal{T}_1} (y_{1t} - \widehat{y}_{1t})^2\big]^{1/2}`, is
reported as :attr:`MLSCResults.pre_rmse`.
...
.. math::
   ...
   y_{sct} = (\alpha_s + \eta_{sc}) \cdot f_t + \varepsilon_{sct},
   \quad
   y_{st} = \frac{1}{C_s} \sum_{c} y_{sct},
...
Differences: :math:`|\widehat{\tau}_{\text{mlsynth}} - \widehat{\tau}_{\text{ref}}|
= 1.5 \times 10^{-6}`,
:math:`|\widehat{\lambda}_{\text{mlsynth}} - \widehat{\lambda}_{\text{ref}}| =
...
:math:`\widehat{\lambda}` does not introduce systematic bias, and ``mlsynth``
