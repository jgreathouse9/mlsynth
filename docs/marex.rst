...
.. list-table:: Authors' R ↔ ``mlsynth`` MAREX
   ...
   * - "treated = smaller set" swap
     - not applied; mlsynth's cardinality is exact and mandatory, so the
       labelling is not free to choose (see below)
   * - Exact permutation test (sum statistic)
     - permutation inference (mlsynth defaults to a mean statistic / sampled
       permutations)

One deliberate divergence, and the reference is entitled to the rule that
``mlsynth`` drops. The authors' base program in ``SCdesign_LazyRun.R`` is two
simplex rows and the disjointness :math:`\mathbf{w}'\mathbf{v} = 0`, solved as
a continuous non-convex QCQP (``vtype = 'C'``) with no selection variable at
all. Exchanging the two weight vectors is feasible there at the same cost, so
the labelling is genuinely free and "treatment regions are the smaller set of
regions" is the tie-break that program needs. Their cardinality routine
enumerates treated sets of every size from 1 up to :math:`K`, defaulting to
:math:`\lfloor N/2 \rfloor`, so the treated group is never the majority there
either and the same rule stays consistent with the search.

``mlsynth``'s program is a different one. A cardinality constraint is mandatory
here, ``m_eq`` is exact, not an upper bound
(:math:`\sum_j z_j = m`, with :math:`w_j \le z_j` and :math:`v_j \le 1 - z_j`),
the cost bound prices :math:`\mathbf{w}` alone, and :math:`m` may exceed half
the panel. Nothing is left to relabel. Carrying the rule across returned three
treated markets for ``m_eq = 6`` on twelve, and they were the control
synthetic's markets -- the group :math:`\sum_j z_j = m` does not constrain and
:math:`\sum_j c_j w_j \le B` does not price. It also negated the estimated
effect, which is the treated synthetic minus the control one. ``mlsynth``
therefore reads the treated group off :math:`\mathbf{w}`, the variable its
program constrains.
