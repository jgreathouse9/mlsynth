...
:math:`\boldsymbol{\mu}_i^\top \mathbf{f}_t` -- common shocks loading
heterogeneously across units -- leaks into the second-stage residual and
contaminates the IV estimate.
...
* a clean pre-period (:math:`t \in \mathcal{T}_1`) during which neither
  the instrument nor the treatment has activated yet, so the data identify
  each unit's exposure to the common factors.

SIV uses that pre-period to fit a per-unit synthetic control that
absorbs the factor loadings :math:`\boldsymbol{\mu}_i`, then runs 2SLS on
the debiased post-period series. The debiased outcome equation has no
...
:math:`\boldsymbol{\mu}_i^\top \mathbf{f}_t`) is sufficient for consistency.
...
Units are indexed by :math:`i \in \mathcal{N} \coloneqq \{1, \ldots, N\}`
(SIV has no single treated unit, so there is no forced :math:`i = 1`); time by
:math:`t \in \mathcal{T} \coloneqq \{1, \ldots, T\}`, with the pre-period
:math:`\mathcal{T}_1 \coloneqq \{t \in \mathcal{T} : t < T_0\}` and the
post-period :math:`\mathcal{T}_2 \coloneqq \{t \in \mathcal{T} : t \ge T_0\}`
(here the intervention is active from :math:`T_0` on, so the split point
:math:`T_0` belongs to :math:`\mathcal{T}_2`). The observed triple
:math:`(y_{it}, R_{it}, Z_{it})` -- outcome, treatment, instrument --
satisfies the structural system

.. math::

   y_{it} &= \theta R_{it} + \boldsymbol{\mu}_i^\top \mathbf{f}_t
            + \varepsilon_{it}, \\
   R_{it} &= (\boldsymbol{\gamma}_i^\top Z_{it} + \eta_{it})
            \cdot \mathbf{1}\{t \ge T_0\}, \\
   Z_{it} &= 0,\ \ t \in \mathcal{T}_1,

with :math:`\mathbf{f}_t \in \mathbb{R}^k` an unobserved factor vector,
:math:`\boldsymbol{\mu}_i \in \mathbb{R}^k` unit loadings, :math:`\theta` the
structural target (the 2SLS coefficient, exposed as ``theta_hat``), and
:math:`(\varepsilon_{it}, \eta_{it})` the outcome and first-stage shocks. The
pre-treatment restriction :math:`Z_{it} = 0` for :math:`t \in \mathcal{T}_1`
(a "sharp intervention" instrument) is what makes the pre-period purely
informative about the factor loadings.
...
\mid Z_{i,1:T}, \eta_{i,1:T}, \boldsymbol{\mu}_i, \{\mathbf{f}_t\}_{t=1}^T] = 0`.
...
:math:`\mathrm{cov}(Z_{it}, \boldsymbol{\mu}_i^\top \mathbf{f}_t)` can be non-zero.

Assumption 3 (factor identification). The pre-period factor matrix
:math:`\mathbf{F}_{\mathcal{T}_1} \in \mathbb{R}^{T_1 \times k}` has rank
:math:`k`, and the donor pool spans :math:`\boldsymbol{\mu}_i` for every focal
unit's loading (the standard SC overlap condition).

*Remark.* Rank :math:`k` lets a linear combination of donor outcomes
exactly reproduce the focal unit's :math:`\boldsymbol{\mu}_i^\top \mathbf{f}_t` over
the pre-period; the overlap condition makes the combination feasible with
...
.. math::
   ...
   \widehat{\mathbf{w}}^{(i)} = \operatorname*{argmin}_{\mathbf{w} \in \Delta^{N-1}}
       \sum_{t \in \mathcal{T}_1} \bigl(y_{it} - \sum_{j \ne i} w_j y_{jt}\bigr)^2,

where :math:`\Delta^{N-1} \coloneqq \{\mathbf{w} \in \mathbb{R}_{\ge 0}^{N-1} :
\|\mathbf{w}\|_1 = 1\}` is the simplex over the :math:`N-1` donor units
(``weight_constraint = "simplex"``) or the :math:`\ell_1` ball
:math:`\{\mathbf{w} : \|\mathbf{w}\|_1 \le C\}` of radius :math:`C`
(``weight_constraint = "l1_ball"``). The fitted weights define
debiased series

.. math::

   \widetilde{y}_{it} \coloneqq y_{it} - \sum_{j \ne i} \widehat{w}^{(i)}_j y_{jt},
   \quad
   \widetilde{R}_{it} \coloneqq R_{it} - \sum_{j \ne i} \widehat{w}^{(i)}_j R_{jt},
   \quad
   \widetilde{Z}_{it} \coloneqq Z_{it} - \sum_{j \ne i} \widehat{w}^{(i)}_j Z_{jt}.

Under Assumptions 1-3 the SC fit absorbs :math:`\boldsymbol{\mu}_i^\top \mathbf{f}_t`
on the pre-period, and -- because all units share the same factor structure
...
.. math::
   ...
   \widetilde{R}_{it} = \pi \widetilde{Z}_{it} + v_{it},
   \qquad
   \widetilde{y}_{it} = \theta \widetilde{R}_{it} + e_{it},
   \qquad t \in \mathcal{T}_2.

The reported ``theta_hat`` is the 2SLS slope from the second equation.
Because the debiased equations no longer contain
:math:`\boldsymbol{\mu}_i^\top \mathbf{f}_t`, partial validity is enough for
...
* ``"projected"`` -- project :math:`\mathbf{y}` onto the instrument space
...
:math:`\widehat\theta_{\mathrm{SIV}} = -0.544`, in the same magnitude
