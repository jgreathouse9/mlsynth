#!/usr/bin/env python3
"""Reference L2-relaxation solve for the ``pda_ppi`` cross-validation.
...
Application: China PPI / real-estate regulation, Section 6.1 (treated ``China``
PPI vs 64 country-control PPI series). Emits the ``== REFERENCE VALUES ==``
block (de-standardised donor weights + fit summary + objective) and
``== SESSION INFO ==``.
"""
...
DATA = ROOT / "basedata" / "china_ppi_long.csv"
