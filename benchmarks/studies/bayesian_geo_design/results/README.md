...
| `unconstrained.json` | `unconstrained.py` | sizes 2..5, honolulu excluded, no forced markets; MVBBSC at 600 / 600 / 2 |
...
The `unconstrained.json` run predates the `n_tied_at_rank1` field the
committed arm records, so that key is absent from it. Every other value is
the measured run under the config above.
