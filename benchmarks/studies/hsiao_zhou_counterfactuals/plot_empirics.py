...
One panel per method. The solid dark line is California's observed per-capita
cigarette sales, the blue line is this replication's counterfactual and the
dashed orange line is the path printed in Table 9. The paper publishes
counterfactuals for the treatment period only, so the pre-period shows the fit
and the post-period shows the fit and the published path side by side.
...
def _panel(ax, years, observed, mine, paper, T0, title, note):
    ...
    ax.plot(years[T0:], paper, color=PAPER_C, lw=2.0, ls=(0, (5, 2)), zorder=3,
            label="fitted, Table 9")
...
def plot_table9(years, observed, fitted, T0):
    ...
    for ax, m in zip(axes.ravel(), order):
        ...
        _panel(ax, years, observed, fitted[m], PAPER[m], T0, m, note)

    lo = min(float(np.min(observed)),
             min(float(np.min(fitted[m])) for m in order),
             min(min(PAPER[m]) for m in order))
    ...
    ax.annotate("Table 9", (years[j], PAPER["PCA"][j - T0]), xytext=(6, -15),
                textcoords="offset points", ha="left", fontsize=8.5,
                color=INK_MUTED)
    ...
    fig.suptitle("California cigarette consumption: observed against the "
                 "fitted counterfactual", fontsize=13.5, color=INK, x=0.011,
                 ha="left", y=0.985)
    fig.text(0.011, 0.935,
             "Shaded from 1989, the first treated year. Proposition 99 passed "
...
def main() -> int:
    ...
    fig = plot_table9(years, Y[:, 0], fitted, T0)
    out = pathlib.Path(__file__).parent / "results" / "table9_observed_vs_fitted.png"
