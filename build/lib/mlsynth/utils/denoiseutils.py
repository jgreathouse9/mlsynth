...
def non_convex_PR( O, Z, r, initial_tau=None, eps=1e-6):
    ...
    for T in range(2000):
        #### SVD to find low-rank M
        M = SVD(O - np.tensordot(Z, tau, axes=([2], [0])), r)  # hard truncation
        #### OLS to get tau
        y = (O - M)[small_index]  # select non-zero entries
        tau_new = Xinv @ (X.T @ y)
        #### Check convergence
...
def DC_PR_auto_rank( O, Z, spectrum_cut=0.002, method='auto'):
    ...
    M, tau, std = DC_PR_with_suggested_rank(O, Z, suggest_r=suggest_r, method='convex')
    y_DCR = M[treatedrow, :]
    ...
    Debiased_Dict = {"Vectors": {"Treated Unit": np.round(observed.reshape(-1, 1), 3),
                                 "Counterfactual": np.round(y_DCR.reshape(-1, 1), 3)},
                     "Effects": {"ATT": round(tau, 3), "Percent ATT": round(100 * tau / np.mean(y_DCR[t1:]), 3)},
                     "CIs": std, "RMSE": round(preRMSE, 3)}
...
def DC_PR_with_suggested_rank( O, Z, suggest_r=1, method='auto'):
    ...
    if method == 'auto':
        ...
        if np.linalg.matrix_rank(M) != suggest_r or np.linalg.norm(
                O - M - np.tensordot(Z, tau, axes=([2], [0]))) > np.linalg.norm(
            O - M1 - np.tensordot(Z, tau1, axes=([2], [0]))):
            M = M1
            tau = tau1
    ...
    CI_dict = {"Lower Bound": round((tau - z_score * SE)[0], 3), "Upper Bound": round((tau + z_score * SE)[-1], 3),
               "SE": SE, "tstat": z_score}
    ...
    attdict, fitdict, Vectors = effects.calculate(observed, y_DCR, t1,
                                                  t2)
...
def RPCA_HQF(M_noise, rak, maxiter, ip, lam_1):
    ...
    for iter in range(maxiter):
        D = M_noise - N
        U = (D @ V.T - lam_1 * U_p) @ np.linalg.inv(V @ V.T - lam_1 * np.eye(rak))
        V = np.linalg.inv(U.T @ U - lam_2 * np.eye(rak)) @ (U.T @ D - lam_2 * V_p)
