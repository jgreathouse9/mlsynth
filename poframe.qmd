...
Academic and business scientists face a fundamental challenge: for any given unit (e.g., a country, region, or individual), we can only observe one outcome at a time [@FPCI]. If a unit receives a single treatment, we see the version of the outcome we see given that the treatment has happened; if it does not, we see the untreated outcome. Formally, for unit $j$ at time $t$ we have:
$$
...
$$
\widehat{\text{ATE}} 
= \frac{1}{N_1} \sum_{j: d_j = 1} y_{jt}
 - \frac{1}{N_0} \sum_{j: d_j = 0} y_{jt},
$$
...
$$
\widehat{\text{ATT}} 
= \frac{1}{N_1} \sum_{j: d_j = 1} y_{jt}(1)
 - \frac{1}{N_1} \sum_{j: d_j = 1} \widehat{y}_{jt}(0),
$$

where $\widehat{y}_{jt}(0)$ denotes the estimated counterfactual outcome for treated unit $j$ in period $t$.
...
::: {.problem title="Potential Outcomes" data-link="#problem-potential-outcomes"}
...
::: {.problem title="ATE vs. ATT" data-link="#problem-ate-vs-att"}
...
::: {.problem title="Causal Interpretation" data-link="#problem-causal-interpretation"}
...
## Counterfactuals and the Role of Controls

Above, I introduced the idea of outcomes $y_{jt}$, but I did not mention where they came from. It turns out that being able to reason about this is central to understanding causal inference models more generally. A useful place to start is the idea of the linear factor model [@lfmtheory].

::: {#assump-lfm .assumption title="Linear Factor Model" data-link="poframe.html#assump-lfm"}
$\forall j \in \mathcal{N}, \: t \in \mathcal{T}_{1}$, pre-treatment outcomes are generated like

$$
y_{jt}(0) = \boldsymbol{\mu}_j^\top \boldsymbol{\lambda}_t + \varepsilon_{jt},
$$

where $\boldsymbol{\lambda}_t \in \mathbb{R}^F$ is a vector of *common factors* affecting all units, $\boldsymbol{\mu}_j \in \mathbb{R}^F$ are *unit-specific factor loadings*, and $\varepsilon_{jt}$ are zero-mean *idiosyncratic errors*. Intuitively, $\boldsymbol{\lambda}_t$ captures time-varying shocks or trends shared across all units, while $\boldsymbol{\mu}_j$ determines how sensitive each unit is to those factors.
:::

If you're anything like me, you're asking what the heck this means practically, so I'll do a small illustration. Suppose there are $F=2$ factor loadings — one capturing geography and another capturing culture. For a treated unit and three donor units, this might  might look like this:

| Unit        | Geography ($\mu_{j1}$) | Culture ($\mu_{j2}$) |
|:------------|----------------------:|--------------------:|
| Treated (1) | 1.20                  | 0.50                |
| Donor A     | 1.05                  | 0.40                |
| Donor B     | 0.80                  | 0.55                |
...
| Time $t$ | Seasonal Factor ($\lambda_{t1}$) | Cyclical Factor ($\lambda_{t2}$) |
|:---------|---------------------------------:|-------------------------------:|
| $t_1$    | 1.0                              | 0.1                            |
| $t_2$    | 0.5                              | 0.3                            |
| $t_3$    | -0.2                             | 0.8                            |
...
Here, $\boldsymbol{\lambda}_t$ varies over time but is *common to all units*, which is why these are called "common factors." Multiplying the factor loadings $\boldsymbol{\mu}_j$ by the time-specific factors $\boldsymbol{\lambda}_t$ shows how each unit responds differently to the same underlying shocks, with $\varepsilon_{jt}$ capturing unit-specific noise. @fig-toylfm gives a slightly extended version of this toy example, plotted so we can see.

```{python}

#| label: fig-toylfm
#| fig-cap: "Simple Toy Factor Model"
...
# Define units and factor loadings
units = ["Treated", "Donor A", "Donor B", "Donor C"]
mu = np.array([
    [1.20, 0.50],  # Treated
    [1.05, 0.40],  # Donor A
    [0.80, 0.55],  # Donor B
...
# Define time periods and time-varying factors
T = 10
lambda_t = np.random.uniform(-1, 1, size=(T, 2))  # random factors for illustration
# You could also use your example factors for the first four periods and random for the rest

# Generate random intercepts for each unit between 500 and 700
intercepts = np.random.uniform(500, 700, size=(len(units), 1))

# Generate synthetic outcomes
data = []
for i, unit in enumerate(units):
    for t in range(T):
        y = intercepts[i] + mu[i, :] @ lambda_t[t, :] + np.random.normal(0, 5)  # small noise
        data.append([unit, t+1, y])

df = pd.DataFrame(data, columns=["Unit", "Time", "Outcome"])

# Plot donors first
for donor in ["Donor A", "Donor B", "Donor C"]:
    subset = df[df["Unit"] == donor]
    plt.plot(subset["Time"], subset["Outcome"], color="grey", linewidth=1, alpha=0.7)

# Plot treated unit on top
treated = df[df["Unit"] == "Treated"]
plt.plot(treated["Time"], treated["Outcome"], color="black", linewidth=3, label="Treated")

# Labels and styling
plt.xlabel("Time")
plt.ylabel("Outcome")
plt.title("Treated vs Donors")
plt.legend()
plt.show()

```

Linear factor models can capture many types of temporal dynamics: seasonal factors repeat periodically within a year, capturing patterns like quarterly sales cycles or flu seasons; cyclical factors represent longer-term economic or social cycles; and periodic or oscillatory factors capture phenomena that recur with a known period, such as day-night cycles or weekly traffic patterns.

It is also possible to define grouped factor models [@relbalscm], where units are organized into groups that share similar baseline structures. In this setting, the overall model can be thought of as a baseline linear factor model, but the details — such as factor loadings or even the number of active factors — may vary within each group. For example, different regions in a country might have a shared set of seasonal and cyclical factors, but the magnitude of their loadings on each factor differs by region. This flexibility allows linear factor models to accommodate complex multivariate data while remaining interpretable: the common factors capture shared variation, the factor loadings capture heterogeneity in responses, and the idiosyncratic errors capture unit-specific randomness.
...
J = 120  # controls
units = J + 1
r = 3  # approx floor(np.log(T))
K = 3  # = r

# Core loadings Lambda_co K x r
Lambda_co = np.random.normal(0, np.sqrt(3/r), size=(K, r))

# w^*_G
w_G = np.zeros(K)
dirich = np.random.dirichlet(np.ones(K-1))  # for entries 2 to K
w_G[1:] = dirich

# epsilon
a = 0.1 / np.sqrt(r)
epsilon = np.random.uniform(-a, a, r)

# lambda_0
lambda_0 = Lambda_co.T @ w_G + epsilon

# Assign groups to controls, groups 0 to K-1
group_assign = np.random.randint(0, K, J)

# Z J x K
Z = np.zeros((J, K))
for i in range(J):
    Z[i, group_assign[i]] = 1

# Lambda_controls J x r
Lambda_controls = Z @ Lambda_co

# alphas
alphas = np.random.uniform(500, 1000, units)

# factors f T x r
f = np.zeros((T, r))
var_stat = 1 / (1 - 0.5**2)  # 4/3
periods = [12, 4, 30]  # Periods for seasonality, cyclic, and periodicity components
for ell in range(r):
    period = periods[ell]
    f[0, ell] = np.random.normal(0, np.sqrt(var_stat)) + np.sin(2 * np.pi * 0 / period)
    for t in range(1, T):
        f[t, ell] = .9 * f[t-1, ell] + np.sin(6 * np.pi * t / period) + np.random.normal(0, 5)
...
for t in range(T):
    for j in range(units):
        if j == 0:
            ...
            lam = Lambda_controls[j-1, :]
        Y[t, j] = alphas[j] + np.dot(lam, f[t, :]) + np.random.normal(0, 3)

plt.figure(figsize=(10, 6))
for j in range(1, units):
    plt.plot(range(T), Y[:, j], color='gray', alpha=0.7, linewidth=0.5)
plt.plot(range(T), Y[:, 0], color='red', linewidth=2, label='Treated Unit')
plt.xlabel('Time')
plt.ylabel('Outcome')
plt.title('Realized Outcomes Over Time')
plt.legend()
...
::: {#assump-afffact .assumption title="Affine Factor Combination" data-link="poframe.html#assump-afffact"}
...
Oftentimes, we assume that the treated unit's factor loadings may be approximated by the loadings of the control group. It says that the treated unit’s long-run structure — captured by its factor loadings $\boldsymbol{\mu}_1$ — can be expressed as a combination of the donor units’ loadings. The scalar $\alpha$ accounts for overall level differences (an intercept or baseline effect), while $\mathbf{M}^\top \mathbf{w}$ represents a weighted blend of donor characteristics.
...
\begin{align}
\boldsymbol{\mu}_1^\top &\in \mathbb{R}^{1 \times F}, &
\mathbf{M} =
\begin{bmatrix}
\boldsymbol{\mu}_2^\top \\
\vdots \\
\boldsymbol{\mu}_{N_0 + 1}^\top
\end{bmatrix}
\in \mathbb{R}^{N_0 \times F}
\end{align}
...
\begin{align}
\boldsymbol{\lambda}_t &\in \mathbb{R}^{F \times 1}, & 
\boldsymbol{\Lambda} =
\begin{bmatrix}
\boldsymbol{\lambda}_1 & \boldsymbol{\lambda}_2 & \cdots & \boldsymbol{\lambda}_T
\end{bmatrix}
...
y_{1T}(0)
\end{bmatrix}
\in \mathbb{R}^{T \times 1}, 
&
\mathbf{Y}_0 &=
\begin{bmatrix} 
y_{21}(0) & \dots & y_{2T}(0) \\ 
\vdots & \ddots & \vdots \\ 
y_{N_0+1,1}(0) & \dots & y_{N_0+1,T}(0)
\end{bmatrix}^\top
\in \mathbb{R}^{T \times N_0}, 
&&
\end{align}



Let [the affine combination assumption](poframe.html#assump-afffact) hold. Then the treated unit’s outcomes can be expressed as a linear combination of the donor outcomes plus an intercept and a residual:

$$
\mathbf{y}_1 = \alpha \mathbf{1}_T + \mathbf{Y}_0 \mathbf{w} + \boldsymbol{\varepsilon}_1^*, 
\quad \text{where} \quad
\boldsymbol{\varepsilon}_1^* = \boldsymbol{\varepsilon}_1 - \mathbf{E}_0 \mathbf{w},
$$
...
::: {.problem title="Linear Factor Model Interpretation" data-link="#problem-linear-factor-model"}
...
a) Explain what the vectors $\boldsymbol{\lambda}_t$ and $\boldsymbol{\mu}_j$ represent.  
b) Give an example of a real-world latent factor that could influence outcomes across units.
:::

::: {.problem title="Affine Factor Combination" data-link="#problem-affine-factor"}
...
#| echo: False
#| label: fig-west-germany-plot-one
...
#| echo: False
#| label: fig-mexhomdata
#| fig-cap: "Chihuahua versus Mexican States"
...
# --- Load data ---
df = pd.read_csv("data/MexicanHomicideData.csv")
...
# --- Separate Chihuahua and other states ---
chihuahua_df = df[df['State'] == 'Chihuahua']
other_states_df = df[df['State'] != 'Chihuahua']
...
for state, group in other_states_df.groupby('State'):
    plt.plot(group['date'], group['Homicide Rate'], color='lightgrey', linewidth=1, alpha=0.7)

# Plot donor mean in blue
plt.plot(mean_other['date'], mean_other['Homicide Rate'], color='blue', linewidth=2, label='Other States (mean)')

# Plot Chihuahua in black
plt.plot(chihuahua_df['date'], chihuahua_df['Homicide Rate'], color='black', linewidth=2.5, label='Chihuahua')

# Labels and formatting
plt.title('Chihuahua vs. Other Mexican States')
plt.xlabel('Date')
...
From a theoretical perspective, each region or store can be thought of as having latent factors that influence sales — for instance, local demographics, store size, historical demand patterns, or competitive intensity. Some regions may load heavily on these factors, while others less so. To understand the campaign’s effect, marketers often rely on holdout studies, sometimes referred to as geolift experiments. In a geolift study, a subset of regions or customer segments is deliberately excluded from the campaign and treated as a control group, while the remaining regions receive the intervention (for example, an increase in media spend compared to the business-as-usual scenario).
...
::: {.problem title="Comparative Case Study Concept" data-link="#problem-comparative-case"}
What is the core idea of a comparative case study? How does it relate to the potential outcomes framework?
:::

::: {.problem title="Selecting Comparison Units" data-link="#problem-comparison-units"}
...
::: {.problem title="Latent Factors in Real-World Examples" data-link="#problem-latent-factors"}
...
::: {.problem title="Marketing Application" data-link="#problem-marketing-application"}
...
::: {.problem title="Bonus Question: Evaluating a Marketing Campaign" data-link="#problem-bonus-marketing"}
...
::: {.proof title="Reconstruction of Treated Outcomes" data-link="#proof-lfm" data-target="#lem-lfm"}

We start from the treated unit’s factor model in vector form:

$$
\mathbf{y}_1 = \boldsymbol{\Lambda} \boldsymbol{\mu}_1 + \boldsymbol{\varepsilon}_1.
$$

Substituting the affine combination $\boldsymbol{\mu}_1 = \alpha \mathbf{1}_F + \mathbf{M}^\top \mathbf{w}$ gives

$$
\mathbf{y}_1 = \boldsymbol{\Lambda} (\alpha \mathbf{1}_F + \mathbf{M}^\top \mathbf{w}) + \boldsymbol{\varepsilon}_1.
$$

Using the distributive property of matrix multiplication:

$$
\mathbf{y}_1 = \alpha \boldsymbol{\Lambda} \mathbf{1}_F + \boldsymbol{\Lambda} \mathbf{M}^\top \mathbf{w} + \boldsymbol{\varepsilon}_1.
$$

Define $\mathbf{1}_T := \boldsymbol{\Lambda} \mathbf{1}_F \in \mathbb{R}^T$, so that the first term becomes
$\alpha \mathbf{1}_T$. For the second term, note that 

$$
\boldsymbol{\Lambda} \mathbf{M}^\top \mathbf{w} = (\mathbf{M} \boldsymbol{\Lambda})^\top \mathbf{w}.
$$ 

By definition, 

$$
\mathbf{Y}_0 = (\mathbf{M} \boldsymbol{\Lambda})^\top + \mathbf{E}_0,
$$ 

which implies $(\mathbf{M} \boldsymbol{\Lambda})^\top = \mathbf{Y}_0 - \mathbf{E}_0$. Substituting gives

$$
(\mathbf{M} \boldsymbol{\Lambda})^\top \mathbf{w} = \mathbf{Y}_0 \mathbf{w} - \mathbf{E}_0 \mathbf{w}.
$$
...
This shows that the treated unit’s outcomes are a linear combination of the donor outcomes plus an intercept and a modified residual. $\blacksquare$
:::
