...
We first test a joint null hypotthesis (namely, that the convex hull constraint is violated). We may write our null hypothesis as $H_0 : w_{j} \in \mathbb{I}, \quad  {\| \mathbf{w} \|\_{1} = 1}$, or, $\mathbf R\beta_0 - q=\mathbf{0}_{2}$. $\mathbf R$ is a matrix where the first and second rows of the first column are 0 and 1 respectively with the latter columns being 1 and 0 respectively.
```math
\left(
\begin{bmatrix}
    0 & \mathbf{1}^{\top}_{N-1} \\
