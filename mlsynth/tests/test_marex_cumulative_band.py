...
pre-period to manufacture one. ``cumulative_band=True`` block-resamples that series
into accumulated paths and reads the half-width off their totals.
...
# The regime the band is built for. The default block is the whole horizon, and a
# block can be at most half the series it is drawn from, so a whole-horizon block
# needs Tb >= 2H. A 120-week pre-period with a 30 percent blank tail and a
# three-month experiment is Tb ~ 36 against H ~ 13, which clears it; this is the
...
def test_different_seeds_give_different_bands():
    """Single-period blocks, because at ``block=0`` a path is one contiguous
    window of the blank series and there are only as many of those as there are
    blank periods, so the quantile lands on the same atom whatever the seed."""
    kw = dict(cumulative_band=True, cumulative_block=1, cumulative_n_sim=300)
    a = _inf(cumulative_seed=4, **kw).cumulative_band
