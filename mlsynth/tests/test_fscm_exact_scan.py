...
tolerance is set by *its* accuracy, not by the code under test.

The invariants that decide correctness are searched, not pinned; see
...
# These moved when #622 put the final weight fit on the exact solver, which is
# a different site from the scan this file changes: the scan chooses the donors,
# `_fit_weights` weights them. The values below are the certified minimiser on
# each selected pool, not a re-fit of whatever the new code printed --
# Proposition 99 at a KKT residual of 2.8e-17 and Basque at 6.7e-16, both unique
# with zero free directions, against a FISTA point whose pre-period SSR is
# higher by 1.9e-05 and 1.1e-10 respectively.
@pytest.mark.parametrize("name,att,donors", [
    ("prop99", -20.15191840,
     {"Montana": 0.416343, "Nevada": 0.254984, "Utah": 0.328673}),
    ("basque", -0.70149517,
     {"Cataluna": 0.839584, "Madrid (Comunidad De)": 0.160416}),
])
