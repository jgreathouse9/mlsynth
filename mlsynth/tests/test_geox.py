"""Tests for GEOX: synthetic difference-in-differences geo experiment design.
...
from mlsynth.config_models import DesignResult, GEOXConfig
from mlsynth.exceptions import MlsynthConfigError, MlsynthDataError
from mlsynth.utils.geox_helpers.engine import (
    placebo_sigma, sdid_att, sdid_fit_once)
from mlsynth.utils.geox_helpers.simulate import inject_effect, simulate_backtest
...
class TestEngineInvariants:
    ...
    def test_matches_the_shipped_sdid_estimator(self, wide):
        """GEOX's engine must be SDID, not a lookalike."""
...
class TestEndToEnd:
    ...
    def test_reproducible(self, panel):
        ...
        a = GEOX(GEOXConfig(**kw)).fit()
        b = GEOX(GEOXConfig(**kw)).fit()
...
class TestMultipleTreatmentSizes:
    ...
    def _design(self, panel, size, **kw):
        ...
        args.update(kw)
        return GEOX(GEOXConfig(**args)).fit()
