"""The GEOX engine seam: a pluggable scoring estimator.

Everything in GEOX except the fit and the p-value is estimator-independent --
...
from mlsynth.utils.geox_helpers.engines import EngineFit, resolve_engine
...
class TestResolution:
    ...
    def test_config_defaults_to_sdid(self, panel):
        cfg = GEOXConfig(df=panel, unitid="location", time="date",
    ...
    def test_config_rejects_an_unknown_engine(self, panel):
        with pytest.raises(MlsynthConfigError, match="engine"):
            GEOXConfig(df=panel, unitid="location", time="date", outcome="Y",
