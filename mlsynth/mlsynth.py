...
from mlsynth.utils.resultutils import effects, plot_estimates, SDID_plot
...
class SDID:
    ...
    def fit(self):
        ...
        # 🔁 Normalize to expected "cohorts" format if missing
        if "cohorts" not in prepped:
            T = prepped["total_periods"]
            T0 = prepped["pre_periods"]
            cohort_time = T0  # first post-treatment period (1-based, matches event time logic)

            prepped["cohorts"] = {
                cohort_time: {
                    "y": prepped["y"].reshape(-1, 1),  # (T, 1)
                    "donor_matrix": prepped["donor_matrix"],  # (T, N0)
                    "treated_indices": [0],  # only one unit
                    "pre_periods": T0,
                    "post_periods": prepped["post_periods"],
                    "total_periods": T,
                }
            }
        results = estimate_event_study_sdid(prepped, placebo_iterations=self.B)
