"""Structured containers for the two-way relaxed SYNDES annealing pipeline."""
...
if TYPE_CHECKING:
    from .structures import SYNDESInputs
...
class RelaxedInference:
    """Permutation-based inference results for the relaxed SYNDES design.
...
class RelaxedSolverResults:
    ...
    inputs : SYNDESInputs, optional
        Preprocessed panel data the solver was run on. Attached by the
        :class:`mlsynth.estimators.SYNDES` orchestrator so the result is
    ...
    :class:`SYNDESDesign` / :class:`SYNDESResults` separation used by the
    mixed-integer SYNDES pipeline. ``mode`` always reports
    ``"two_way_global_annealed"`` so plotting and dispatch code can branch on
    it uniformly with :class:`SYNDESResults`.
    """
    ...
    def mode(self) -> str:
        ...
        return "two_way_global_annealed"
