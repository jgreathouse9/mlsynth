"""Fully Bayesian synthetic-control posterior solver.

Core Gaussian-conjugate posterior over the synthetic-control weights, used by
the PCR-SC Bayesian variant (:func:`..solve_bayesian`). Relocated from the
former shared ``mlsynth.utils.bayesutils`` module: it has a single consumer
