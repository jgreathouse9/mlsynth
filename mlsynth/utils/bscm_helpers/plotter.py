"""Observed vs counterfactual plot for BSCM, with posterior credible band."""
...
from .structures import BSCMResults


def plot_bscm(results: BSCMResults, title: str | None = None) -> None:
    """Plot observed series, BSCM counterfactual, and pointwise credible band.

    One solid black series for the observed outcome, a dashed blue
    posterior-mean counterfactual, a shaded credible band, and a vertical
    line at ``T_0``.
    """
    ...
    ax.plot(
        ...
        label=f"BSCM ({results.posterior.prior}) counterfactual",
    )
    ax.fill_between(
        time,
    ...
    if title is None:
        ...
        title = f"BSCM counterfactual: {inputs.treated_unit_name}{att_str}"
