...
def cwz_cumulative_per_unit(
    ...
    *, fixedeff: bool, time_cohort: bool, nu_used: float, lam: float,
    ...
    the ``T`` cyclic shifts of the residual path rather than a disjoint split of
    ...
    Two construction details are load-bearing, both established by measurement
    rather than argument; the full account is in
    ...
    the program. That is why ``nu_used`` is a parameter rather than a default.
    ...
    rather than by the data, and that end is returned as infinite. Reporting the
