...
def _init_cvxpy_variables(N, K):
    ...
    z = cp.Variable((N, K), nonneg=True)  # Relaxed to continuous [0,1]
    return w, v, z
...
def _build_constraints(w, v, z, M, cluster_members, cluster_labels,
    ...
    for k_idx, lab in enumerate(cluster_labels):
        ...
        for j in members:
            ...
            constraints += [z[j, k_idx] <= 1]  # Added for relaxation
...
def _build_objective(Y_fit, Xbar_clusters, cluster_members, w, v, z,
    ...
    # Optional integrality penalty for z (non-convex if zeta > 0)
    if zeta > 0:
        for k_idx in range(K):
            members = cluster_members[k_idx]
            obj_terms.append(zeta * cp.sum(z[members, k_idx] * (1 - z[members, k_idx])))

    return cp.Minimize(cp.sum(obj_terms))


def _post_hoc_discretize(
    w_opt, v_opt, cluster_members, cluster_labels, 
    m_eq=None, m_min=None, m_max=None, 
    trim_threshold=1e-2, Y_fit=None, Y_blank=None
):
    """
    Post-hoc discretization of relaxed SCM weights.
    - Enforces m_eq, m_min, m_max constraints.
    - Selects top-m units per cluster by weight magnitude.
    - Renormalizes selected weights to sum to 1.
    - Computes RMSE on blank periods if available.

    Returns
    -------
    w_discrete, v_discrete : np.ndarray
        Post-hoc treated/control weights.
    selected_treated, selected_control : list[list[int]]
        Indices of selected treated/control units per cluster.
    rmse_blank : list[float or None]
        Blank-period RMSE per cluster.
    """
    K = len(cluster_members)
    N = w_opt.shape[0]
    ...
    for k_idx, lab in enumerate(cluster_labels):
        ...
        # --- determine number of treated to select ---
        m_eq_k = _get_per_cluster_param(m_eq, lab)
        m_min_k = _get_per_cluster_param(m_min, lab, default=1)
        m_max_k = _get_per_cluster_param(m_max, lab, default=len(members) // 2)

        if m_eq_k is not None:
            m_select = int(m_eq_k)
        else:
            # heuristic: target the number of non-zero weights, but enforce bounds
            nonzero = np.count_nonzero(w_k)
            if nonzero == 0:
                nonzero = m_min_k
            m_select = min(max(nonzero, m_min_k), m_max_k)

        # --- select top m treated ---
        ...
        if w_sel.sum() > 0:
            ...
            w_k[top_indices] = w_sel / w_sel.sum()
        else:
            # fallback uniform
            w_k = np.zeros_like(w_k)
            w_k[top_indices] = 1.0 / len(top_indices)

        w_discrete[members, k_idx] = w_k

        # --- controls: all remaining members ---
        control_idx = np.setdiff1d(members, treated_idx)
        selected_control[k_idx] = control_idx.tolist()
        v_sel = v_k[np.isin(members, control_idx)]
        if v_sel.sum() > 0:
            v_k = np.zeros_like(v_k)
            v_k[np.isin(members, control_idx)] = v_sel / v_sel.sum()
        else:
            # fallback uniform over controls
            if len(control_idx) > 0:
                v_k = np.zeros_like(v_k)
                v_k[np.isin(members, control_idx)] = 1.0 / len(control_idx)
        v_discrete[members, k_idx] = v_k

        # --- compute RMSE on blank periods ---
...
def SCMEXP_REL(
    ...
    solver=cp.ECOS,
    verbose=False,
    zeta=0.0,  # Integrality penalty, 0 for convex
    trim_threshold=1e-2  # For post-hoc
):
    """
    Relaxed Clustered Synthetic Control for Experimental Design (SCMEXP_REL).

    Relaxes the MIQP to QP/QCQP by making z continuous [0,1], solves, then applies post-hoc discretization.
    Supports swapping based on sparsity.

    ... (rest of docstring similar to original, add notes on relaxation and post-hoc)
    """
    # --- validate inputs ---
    ...
    y_syn_treated_clusters_rel = [Y_full_T @ w_opt_rel[:, k_idx] for k_idx in range(K)]
    y_syn_control_clusters_rel = [Y_full_T @ v_opt_rel[:, k_idx] for k_idx in range(K)]

    # --- post-hoc discretization ---
    w_opt, v_opt, selected_treated, selected_control, rmse_blank = _post_hoc_discretize(
    ...
    # cluster RMSE (post-hoc)
    rmse_cluster = []
    for k_idx in range(K):
        treated_idx = selected_treated[k_idx]
    ...
    result = {
        "df": Y_full,
        "w_opt_rel": w_opt_rel,  # Relaxed
        "v_opt_rel": v_opt_rel,
        "z_opt_rel": z_opt_rel,
        "w_opt": w_opt,  # Post-hoc
        "v_opt": v_opt,
        "selected_treated": selected_treated,
