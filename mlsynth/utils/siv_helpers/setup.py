...
def project_outcome_pre(
    ...
    if loading is None:
        ...
        # Use the top-1 left singular vector to recover Z_i up to sign
        # and scale: under the shift-share factor structure
        # Z_it = Z_i * g_t the rank-1 SVD has u = Z_i / ||Z_i||. This
        # is more robust than averaging across t when g_t has mean
        # near zero (which produces a vanishingly small mean(Z_post)
        # despite the true loadings being O(1)).
        U, S, _ = np.linalg.svd(Z_post, full_matrices=False)
        loading_vec = U[:, 0] * S[0]
    else:
