...
class SLConfig(BaseEstimatorConfig):
    ...
    eta: Optional[float] = Field(default=None, ge=0.0, description="Learning rate for the exponential weights of Equation 12. None (default) takes the paper's 1/(sqrt(T) var(y)), which evaluates to 51.43 on their panel against the best_eta <- 50 their .Rhistory hard-codes. That value averages instead of selecting: effective_k comes out at 3.88 of 4, and concentrating on the best expert needs eta 17 to 60 times larger. Set it explicitly to move along that interpolation; 0 is the simple average. Read effective_k on the result to see which regime a fit is in.")
