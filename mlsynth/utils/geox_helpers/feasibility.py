"""Up-front, itemised feasibility audit for GEOX market selection.
...
(same shape, same contract: it reports; it never relaxes a constraint the analyst set), re-implemented label-based here so GEOX owns its constraint
...
def _raise(problems: List[str]) -> None:
    if problems:
        raise MlsynthConfigError(
            "GEOX design is infeasible -- the binding constraint(s):\n  - "
            + "\n  - ".join(problems)
        )
