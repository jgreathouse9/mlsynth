...
def test_treat2(treatment_matrix: np.ndarray):
    """
    ...
    t2 = int(np.linalg.norm(treated_column, ord=1))  # Explicitly convert to integer
    assert t2 == np.linalg.norm(treated_column, ord=1), f"The l1 norm of the treated column is not an integer."
    ...
    t1 = int(np.size(treated_column) - np.count_nonzero(treated_column))  # Explicitly convert to integer
    total_periods = t2 + t1
...
def dataprep(df, unitid, time, outcome, treat):
    ...
    donor_df = Ywide.drop(Ywide.columns[test_results["Treated Index"][0]], axis=1)
...
def prepare_data(df, unitid, time, outcome, treat):
    t2, t1, trcolnum, t= test_treat2(df.pivot(index=time, columns=unitid, values=treat).to_numpy())
    Ywide = df.pivot(index=time, columns=unitid, values=outcome)
