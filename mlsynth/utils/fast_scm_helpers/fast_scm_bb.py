...
def branch_and_bound_topK(
    ...
    brute_force_runtime_sec: Optional[float] = None,
) -> Dict[str, Any]:
    ...
    node_fraction = stats["nodes_visited"] / total_nodes if total_nodes else 0.0
    subset_fraction = (
        stats["subsets_evaluated"] / total_subsets if total_subsets else 0.0
    )

    prune_rate = (
        stats["branches_pruned"] / stats["branches_considered"]
        if stats["branches_considered"] > 0
        else 0.0
    )
