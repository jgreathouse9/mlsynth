"""Observed vs MTGP counterfactual plot with the posterior credible band."""
...
from .structures import MTGPResults


def plot_mtgp(results: MTGPResults, title: str | None = None) -> None:
    """Plot the observed series, the MTGP counterfactual, and its credible band."""
    ...
    ax.plot(time, inf.counterfactual_mean, color="tab:red", linestyle="--",
            linewidth=1.8, label="MTGP counterfactual")
    ...
    if title is None:
        ...
        title = f"MTGP counterfactual: {inputs.treated_unit_name}{att_str}"
