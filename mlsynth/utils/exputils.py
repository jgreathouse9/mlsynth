...
def SCMEXP(
    ...
    if costs is not None:
        ...
        if isinstance(budget, (int, float)):
            # Evenly split scalar budget across clusters
            budget = {lab: budget / K for lab in cluster_labels}
        elif isinstance(budget, dict):
            # Check that all clusters have a budget entry
            for lab in cluster_labels:
                if lab not in budget:
                    raise ValueError(f"budget missing entry for cluster '{lab}'.")
        else:
            raise TypeError("budget must be a scalar or a dict if costs are provided.")
    ...
    # --- precompute D1 and D2 for penalized designs ---
    D1 = np.zeros((N, K))
