...
from ..utils.estutils import NSCcv, NSC_opt
...
class NSC:
    ...
    ... )
    >>> estimator = NSC(config=nsc_config)
    >>> # Results can be obtained by calling estimator.fit()
    >>> # results = estimator.fit() # doctest: +SKIP
    """

    def __init__(self, config: NSCConfig) -> None: # Changed to NSCConfig
        """
        Initializes the NSC estimator with a configuration object.
        ...
        config : NSCConfig
            ...
            The fields include:
                ...
                counterfactual_color : str, default="red"
                    ...
                    but `NSCConfig` defines it as `str`).
                treated_color : str, default="black"
                    Color for the treated unit line in plots.
                a_search_space : Optional[List[float]], default=None
                    Optional list of values for hyperparameter 'a' for cross-validation.
                b_search_space : Optional[List[float]], default=None
                    Optional list of values for hyperparameter 'b' for cross-validation.
        """
        if isinstance(config, dict):
            config =NSCConfig(**config)  # convert dict to config object
        ...
        self.save: Union[bool, str] = config.save # Aligned with BaseEstimatorConfig
        self.a_search_space: Optional[List[float]] = config.a_search_space
        self.b_search_space: Optional[List[float]] = config.b_search_space

    def _create_estimator_results(
        self, aggregated_fit_results: Dict[str, Any]
    ) -> BaseEstimatorResults:
        """
        Constructs a BaseEstimatorResults object from raw NSC outputs.
    ...
    def fit(self) -> BaseEstimatorResults:
        """
        Fits the Nonlinear Synthetic Control (NSC) model to the provided data.
