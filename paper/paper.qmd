...
experiment = MAREX({
    ...
    "T0": 100, "blank_periods": 28, "T_post": 15,   # fit 100, 28 blank, 15 experimental
...
effect (`{python} f"p = {float(pf.p_value):.2f}"`). Abadie and Zhao report a
closely analogous result on this panel, a design that tracks well and a placebo
$p$-value of $0.933$ that does not reject. The small numerical gap is expected:
the design problem @eq-marex is non-convex with a flat objective near the optimum,
so the open SCIP solver and the authors' Gurobi reach different but
comparably-balanced two-store designs. `mlsynth` cross-validates against the
authors' own design code (their open cardinality-constrained solver) on a
tractable store subset in the durable benchmark `marex_walmart`, where the two
agree to within solver tolerance. The conclusion is the one the placebo is meant
to deliver: the design manufactures no effect where there is none.
