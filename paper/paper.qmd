...
# And of the benchmark cases behind them, read off the registry the suite runs.
# Parsed, not imported: importing the registry would import every case module
# and the reference toolchains they reach for.
def _n_benchmark_cases():
    import ast

    here = Path.cwd().resolve()
    for base in [here, *here.parents]:
        registry = base / "benchmarks" / "registry.py"
        if registry.exists():
            ...
            raise ValueError(f"no CASES mapping in {registry}")
    raise FileNotFoundError(f"could not locate benchmarks/registry.py above {here}")


n_benchmark_cases = _n_benchmark_cases()
...
Synthetic control can also be used for
*experimental design*, where a weighting logic chooses which units to treat, and
...
This proliferation of methods carries costs to the practitioner, however. Each new SC variant typically ships as replication
...
frame and compare them. `mlsynth` aims to make this catalog accessible, free, and simple to call. The
...
(for example <https://mlsynth.readthedocs.io/en/latest/sdid.html>).
...
separately, by a benchmark suite of `{python} n_benchmark_cases` small re-runnable cases that
