...
(@tbl-basque-weights). The nested search puts most of its weight on Cataluna; the
...
fmt = lambda v: "" if pd.isna(v) else f"{v:.3f}"      # blank for unused donors
(pd.concat([positive_weights(mscmt, "MSCMT (nested)"),
            positive_weights(malo, "Malo (bilevel)")], axis=1)
   .rename_axis("donor region")
   .apply(lambda col: col.map(fmt)))
```
