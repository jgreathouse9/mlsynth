...
ax.plot(years, observed, color="black", lw=1.7, zorder=5,
        label="Basque Country (observed)")
for res, color, ls, name, dx in [(mscmt, "C0", "--", "MSCMT nested solver", -0.25),
                                  (malo, "C3", "-.", "Malo bilevel solver", 0.25)]:
    cf = np.asarray(res.time_series.counterfactual_outcome, dtype=float)
    ax.plot(years, cf, color=color, ls=ls, lw=1.2, label=name)
    band = res.inference.details          # per-period Cattaneo-Feng-Titiunik bounds
    post_years = np.array(band["periods"], dtype=float)
    cf_post = cf[np.isin(years, band["periods"])]
    ax.errorbar(post_years + dx, cf_post,
                yerr=[cf_post - band["counterfactual_lower"],
                      band["counterfactual_upper"] - cf_post],
                fmt="none", ecolor=color, elinewidth=0.7, capsize=1.5, alpha=0.8)
