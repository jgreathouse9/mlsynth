...
abstract: |
  ...
  it with three illustrations, one per regime. The first refits Abadie and
  Gardeazabal's Basque study with two solvers, a nested search and a provably
  better bilevel optimizer that no other package carries together, with Cattaneo,
...
Why weights that reproduce the pre-treatment vector should also recover the
post-treatment counterfactual is a separate, statistical question. The rationale
the literature offers is a low-dimensional factor model,
$$
...
common time effect, and $\varepsilon_{it}$ is idiosyncratic noise. Units differ
only through the handful of latent loadings $\boldsymbol{\lambda}_i$, so if the
treated unit's loadings are a combination of the donors', weights that reproduce
its pre-treatment outcomes also reproduce its loadings, and hence its untreated
path after treatment. We take this as the standard justification, refer readers to
the source papers, and note that the library does not commit to it: several members
of the family relax or replace it. The synthetic control
$$
...
selected or weighted, how the common time structure is handled), not a list of
unrelated algorithms. Every one still consumes the same inputs ($\mathbf{Y}_0$,
$\mathbf{y}_1$, $T_0$) and returns the same output, a counterfactual path and its
gap, which is why the whole family fits behind one interface.
...
att_mscmt, att_malo = float(mscmt.effects.att), float(malo.effects.att)
gap_pct = 100 * (att_malo - att_mscmt) / att_mscmt
```

```{python}
#| label: fig-basque
#| fig-cap: "The Abadie-Gardeazabal Basque study fit twice, changing only the solver. Observed Basque per-capita GDP (black) against the synthetic control from the MSCMT nested differential-evolution search (blue, dashed) and from the Malo et al. bilevel solver (red, dash-dotted). The solid vertical line marks 1970, the start of the treatment window. The two counterfactuals are close in level but rest on different donor weights (@tbl-basque-weights), and the bilevel solver fits the pre-treatment window better."

cf_mscmt = np.asarray(mscmt.time_series.counterfactual_outcome, dtype=float)
cf_malo = np.asarray(malo.time_series.counterfactual_outcome, dtype=float)

fig, ax = plt.subplots(figsize=(5, 3.2))
ax.plot(years, observed, color="black", lw=1.6, label="Basque Country (observed)")
ax.plot(years, cf_mscmt, color="C0", ls="--", lw=1.3, label="MSCMT nested solver")
ax.plot(years, cf_malo, color="C3", ls="-.", lw=1.3, label="Malo bilevel solver")
...
(@tbl-basque-weights). The nested search puts most of its weight on Cataluña; the
bilevel solver drops Cataluña entirely, promotes Madrid to the largest donor, and
...
bilevel solution attains a lower pre-treatment fit loss (RMSE
`{python} f"{pre_rmse(malo):.4f}"` against `{python} f"{pre_rmse(mscmt):.4f}"`). The
better fit moves the headline number. The nested solver puts the average GDP
shortfall from terrorism at `{python} f"{abs(att_mscmt):.2f}"` thousand USD per
capita; the bilevel optimum, fitting the pre-period better, puts it
`{python} f"{abs(gap_pct):.0f}"`% larger at `{python} f"{abs(att_malo):.2f}"`, and
...
@tbl-basque-weights reads them side by side.

```{python}
#| label: tbl-basque-weights
#| tbl-cap: "Donor weights from the two solvers (`weights.donor_weights`), zero-weight regions omitted. The nested and bilevel searches select different regions, and weight their shared donors differently, for the same treated unit."
def positive_weights(res, name):
    s = pd.Series(res.weights.donor_weights, name=name)
    return s[s.abs() > 1e-3]

(pd.concat([positive_weights(mscmt, "MSCMT (nested)"),
            positive_weights(malo, "Malo (bilevel)")], axis=1)
   .round(3).fillna("").rename_axis("donor region"))
```

That side-by-side is the point. The disagreement between a widely used nested
solver and the provably better bilevel one is visible only because `mlsynth` hosts
both, each cross-validated against its reference (the R package `MSCMT` and
@malo2024computing's `scm.corner`), behind one estimator and one `backend` keyword.
...
largely incidental: most of these estimators run the same computation, a
treated-unit vector and a donor matrix through an optimization to weights, a
counterfactual, and a gap, differing only in how they constrain or denoise that
fit. `mlsynth` takes that observation seriously, placing more than forty estimators
...
Estimation becomes a genuine comparison: on Abadie and Gardeazabal's Basque study,
a nested solver and a provably better bilevel one, which exist together in no other
package, reach different synthetic controls from the same data, each by a one-field
change against an unchanged data frame. Relaxing the
