...
should exist and *how* to recover them. The library does not adjudicate between the
rationales, and its estimators were built on different ones, so it is worth setting
the main four side by side, because they explain how one interface can host methods
with such different theory.
...
- Proximal causal inference [@proximaltheory] keeps the latent factors but drops
  ...
  It reads the donor outcomes as error-laden proxies for the unobserved confounders
  $\mathbf{f}_t$, and repurposes donors that do not enter the synthetic control as
  instruments that form moment equations identifying the weights. Existence then
  follows from a bridge condition rather than a hull condition, which extends
  synthetic control to poor pre-treatment fit and to binary or count outcomes, and
  replaces the linear factor assumption with a nonparametric one.
