...
disagreement between a widely used nested solver and the provably better bilevel one
is attributable to the solver alone rather than to two reconciled implementations.
...
correct the measurement error: this is the proximal-inference family, and it is the
part of the catalog a Python user could not run until now. The single-proxy method
of @ParkTchetgenTchetgen2025 shipped only as [an R package](https://github.com/qkrcks0218/SPSC);
the spillover-detection screen of @oriordan2025spillover had no public
implementation in any language. `mlsynth` carries both, and behind the one
...
#| label: fit-proximal
prox = PROXIMAL({
    "df": panic, "treat": "Panic", "time": "date", "outcome": "prc_log",
    "unitid": "ID", "vars": {"donorproxies": ["bid_itp"], "surrogatevars": ["ask_itp"]},
    "donors": normal, "surrogates": affected, "methods": ["PI", "SPSC"],
    "display_graphs": False}).fit()
att = prox.att_by_method()                       # two-proxy PI and single-proxy SPSC

spot = SPOTSYNTH({                               # screen the pool, then debias
    "df": panic, "outcome": "prc_log", "treat": "Panic", "time": "time",
    "unitid": "ID", "display_graphs": False, "selection": "S1",
    "forecast": "loo", "inference": "frequentist", "debias": True}).fit()
...
violently affected trust. The cross-method agreement and this ranking are pinned as
a durable benchmark (`benchmarks/cases/spotsynth_panic1907.py`). The point the
illustration makes is the one the package is built to make: a method that lived only
in R, and a method that lived nowhere, read the same panel through the same
interface and can be checked against each other.
...
no-interference assumption becomes a measurable sensitivity check: four
spillover-aware estimators, otherwise four separate codebases in another language,
run across two case studies behind one keyword, their disagreement a quantity to
read rather than four outputs to reconcile. And design becomes available at all: MAREX, the first
